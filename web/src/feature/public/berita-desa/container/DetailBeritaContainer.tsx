...
/** Berita terkait: kurasi manual bila ada, jika tidak ambil 3 berita lain terbaru. */
function ambilBeritaTerkait(berita: Berita, semua: Berita[]): Berita[] {
    if (berita.terkaitIds?.length) {
        return berita.terkaitIds
            .map((id) => semua.find((item) => item.id === id))
            .filter((item): item is Berita => Boolean(item));
    }

    return semua.filter((item) => item.id !== berita.id).slice(0, 3);
}
...
export default async function DetailBeritaContainer({ id }: { id: string }) {
    ...
    const berita = semua.find((item) => item.id === id);
    if (!berita) notFound();

    const beritaTerkait = ambilBeritaTerkait(berita, semua);
